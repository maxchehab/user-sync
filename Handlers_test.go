...
func TestEventHandler(t *testing.T) {
	Constants = models.EnvConstants{
		Token: "UCpbtuZdEDQ7ewruOXvd0QoN",
	}
	...
	t.Run("Routes to URLVerificationHandler", func(t *testing.T) {
		...
		request, _ := http.NewRequest("POST", "/", strings.NewReader(`{"token":"UCpbtuZdEDQ7ewruOXvd0QoN","type":"url_verification"}`))
	...
	t.Run("Routes to UserChangeHandler with user_change event", func(t *testing.T) {
		...
		request, _ := http.NewRequest("POST", "/", strings.NewReader(`{"token":"UCpbtuZdEDQ7ewruOXvd0QoN","event":{"type": "user_change"},"type": "event_callback"}`))
		EventHandler(httptest.NewRecorder(), request)
	...
	t.Run("Routes to UserChangeHandler with team_join event", func(t *testing.T) {
		...
		request, _ := http.NewRequest("POST", "/", strings.NewReader(`{"token":"UCpbtuZdEDQ7ewruOXvd0QoN","event":{"type": "team_join"},"type": "event_callback"}`))
		EventHandler(httptest.NewRecorder(), request)
...
func TestURLVerificationHandler(t *testing.T) {
	...
	t.Run("Valid token and challenge", func(t *testing.T) {
		recorder := httptest.NewRecorder()
		request := []byte(`{"token":"UCpbtuZdEDQ7ewruOXvd0QoN","challenge":"hzYxsxj1xJ7EFOalUU9j5Rmeml62UtGuM7EQlmxaAPCAV2eoZDRC","type":"url_verification"}`)
